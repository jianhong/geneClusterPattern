...
#' @param label_size The label cex value. default is 1.
#' @return NULL
#' @importFrom trackViewer trackList optimizeStyle setTrackViewerStyleParam viewTracks setTrackStyleParam
#' @importFrom methods new
plotOneTrack <- function(gr, region, species, scaleBar=FALSE, label_size=1){
  ...
  if(length(gr)){
    ...
    setTrackStyleParam(trackList[[1]], 'ylabgp', list(cex=label_size))
    if(scaleBar){
      setTrackViewerStyleParam(viewerStyle, "xaxis", TRUE)
      setTrackViewerStyleParam(viewerStyle, "margin", c(.1, .01, .01, .05))
    }else{
      setTrackViewerStyleParam(viewerStyle, "xaxis", FALSE)
      setTrackViewerStyleParam(viewerStyle, "margin", c(.01, .01, .01, .05))
    }
